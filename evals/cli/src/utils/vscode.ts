...
`,
	...
	const settings = {
		...
		"workbench.view.extension.ninjatech.ninja-dev-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.ninjatech.ninja-dev-ActivityBar",
			when: "viewContainer.workbench.view.extension.ninjatech.ninja-dev-ActivityBar.enabled",
		},
	]
	...
	const args = [
		...
		"--command",
		"workbench.view.extension.ninjatech.ninja-dev-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.ninjatech.ninja-dev-ActivityBar');
		}, 5000);
	`
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						const extension = vscode.extensions.getExtension('ninjatech.ninja-dev');
						if (!extension) {
						...
						await vscode.commands.executeCommand('workbench.view.extension.ninjatech.ninja-dev-ActivityBar');
